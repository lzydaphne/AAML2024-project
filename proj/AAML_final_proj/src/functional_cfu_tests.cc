...
void do_matmul_num(int test_num) {
  ...
  */
  for (int by = 0; by < 4; by++) {    // Block row
    for (int bx = 0; bx < 4; bx++) {  // Block column

      // 1. Load matrix B - 4x4 block (row-major)
      for (int i = 0; i < 4; i++) {
        int b_base = (by * 4 + i) * 4;
        cfu_op1(0, B_arr[test_num][b_base], B_arr[test_num][b_base + 1]);
        cfu_op1(0, B_arr[test_num][b_base + 2], B_arr[test_num][b_base + 3]);
      }

      // 2. Load matrix A - 4x4 block (column-major)
      for (int i = 0; i < 4; i++) {
        int a_base = bx * 16 + i;
        cfu_op0(0, A_arr[test_num][a_base], A_arr[test_num][a_base + 4]);
        cfu_op0(0, A_arr[test_num][a_base + 8], A_arr[test_num][a_base + 12]);
      }
      ...
      for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
          C_arr[bx * 4 + i][by * 4 + j] = cfu_op3(0, 0, 0);
        }
      }
    }
  }
